...
assets/prompt-vibe-coding.png      lo screenshot del prompt (slide 6)
assets/codice-senza-memoria.png    il codice con la costante misteriosa (slide 10)
...
|---|---|---|
| 00 · Apertura | 1-4 | ~7 min |
| 01 · Overview dei tool | 6 | ~3 min |
| 02 · Vibe coding e debito cognitivo | 6-13 | ~12 min |
| 03 · BMAD | 14-32 | ~21 min, di cui ~8 di demo |
| 04 · Trasferire conoscenza agli agenti | 33-37 | ~12 min |
| 05 · Chiusura | 38-40 | ~4 min |

Il capitolo 03 sta in 19 slide (14-32), di cui **nove sono la demo** (24-32). Non è più una
...
erano nero vuoto. Sta nella slide 6 e vale come definizione visiva del vibe coding.

`assets/codice-senza-memoria.png` è il codice Java con il commento datato e la costante
`291048`. Sta nella slide 10 ed è la prova concreta del punto: il codice dice cosa fa, il
...
## Il blocco sulla memoria (slide 10-12)

La **10** mostra codice reale con una costante che nessuno sa spiegare, e chi paga il conto:
...
La **11** taglia la scorciatoia che tutti hanno in testa: tenere tutto in una chat sola. Il
...
La **12** riformula la domanda — non «AI sì o AI no» ma «AI con quale processo» — e mette
...
## Il capitolo 03, slide per slide (14-32)

| Slide | Cosa fa |
|---|---|
| 14 | il flusso ufficiale di BMAD: dal PRD alla story rivista, un artefatto per freccia |
...
| 23 | il diagramma del loop coi checkpoint umani |
| 24 | il caso della demo: un servizio del 2022, il 41%, i sei display in corridoio |
| 25-31 | **la demo**, sette momenti: menu · checkpoint · **contesto** · story · artefatti · review · verdetto |
| 32 | pitfall e strategie, la slide che il pubblico fotografa |

La **22** (i cinque insieme) apre la strada al diagramma del loop della **23**, che riassume
...
Quel taglio però lasciava un buco fra la **26** e la story: la demo passava dal product brief
...
Adesso il buco lo chiudono due cose. La **27** mostra il gesto che mancava, e lo fa per
...
terminale della **28** — venti secondi parlati, non un momento in più. Il copione ha
entrambe per esteso.

La **13** chiude il capitolo 02 con la tesi in una riga: BMAD non aggiunge intelligenza
all'AI, aggiunge struttura al processo. È la cerniera, non l'apertura del capitolo 03.

La **14** apre il capitolo 03 con il flusso ufficiale: PRD → architettura → epiche e story,
...
I primi due posti dell'elenco tornano dal vivo nella **27**: la decisione entra in
`project-context.md`, e quel file sta nei `persistent_facts` di ogni agente. La 16 dice
*dove*, la 27 fa vedere *come*. Se sposti una delle due, l'altra perde l'aggancio.

Nota che la slide cita PRD e architettura prima che il pubblico sappia chi li scrive: se in
prova stona, valuta di spostarla dopo il blocco identikit (16-20), quando John e Winston sono
...
- **«dalla richiesta agli artefatti»** (quattro conversazioni, quattro documenti,
  «la conversazione è usa e getta»): stessa tesi già detta alla 11 e alla 15, e ridimostrata
  dal vivo dentro la demo («cosa resta dopo», slide 29). Tre ripetizioni della stessa frase
...
sulla 13 sono rientrati: il titolo è «Il framework in una slide» e il corpo porta la
...
|---|---|---|
| 24 | il caso | **brownfield**: il servizio c'è dal 2022, arriva una feature. Il 41% e i sei display |
| 25 | attivazione e menu | tre colonne, e la terza è **lo skill che parte**: `CB` → `bmad-product-brief` |
| 26 | la feature rimandata al problema | «quella che mi hai dato non è un problema, è già una soluzione». Poi il bivio, e la decisione di **scrivere fuori dal servizio** |
| 27 | la decisione sale nel contesto | la leva punitiva tolta col motivo scritto, poi party mode: la regola esce dal brief ed entra in `project-context.md` |
| 28 | la story è un contratto | AC numerati, task che citano l'AC — e il vincolo E3 sul display piantato nelle Dev Notes |
| 29 | cosa resta dopo | `DS`, il testo del display **scritto a mano** e seminato di sfuggita, la definition of done, lo stato in `sprint-status.yaml` |
| 30 | tre reviewer, tre contesti | Blind / Edge Case / Acceptance Auditor. Uno approva, l'altro trova il titolo di un colloquio sullo schermo |
| 31 | corretto per la story | il triage in quattro bucket, il **prima/dopo sullo schermo in corridoio**, e «come ti è sfuggito?» |
| 32 | pitfall e strategie | sei errori e sei contromisure, in parallelo |
...
Le otto slide sono **in ordine di tempo** e percorrono il loop della 23 invece di
...
è quella che pianta il vincolo sul display, e senza di lei la 30 non ha niente da
raccogliere. Le 26, 27, 28 e 30 si raccontano a voce in un minuto — la 27 è la prima a
...
reale. Il tag della 24 dice «caso di esempio» apposta. La distinzione da fare in sala è
...
| Slide | Cosa fa |
|---|---|
| 33 | il limite che resta: lo stesso loop, col buco del contesto al centro |
| 34 | l'idea: file di contesto piatto contro grafo navigabile — e le fonti che non stanno nel repo |
| 35 | gli strumenti: Graphify, llm-wiki, le alternative, più il consiglio |
| 36 | Graphify in pratica: cosa gli dai in pasto e cosa ti risponde |
| 37 | il buco riempito: chi legge dal grafo e chi ci scrive |

La **34** dice che call, mail e note di corridoio diventano nodi come tutto il resto; la
**36** lo mostra: quattro `ingest`, una domanda vera («posso cambiare il separatore
decimale?»), e una risposta che cita quattro fonti di cui tre fuori dal repo. È il pay-off
del punto piantato alla 12 — le decisioni prese in pausa caffè non esistono finché qualcuno
...
- **slide 39** — link e QR code
- **slide 40** — nome e contatto interno

## La slide da curare

La **23** (il loop BMAD) è quella che regge il talk. Compare tre volte con lo stesso disegno
