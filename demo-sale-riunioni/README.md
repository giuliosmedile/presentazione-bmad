...
│   │       ├── 1-1-…-di-una-sala.md      done, con Dev Agent Record e Review Findings
│   │       ├── 1-2-check-in-…md          in progress, si ferma su una domanda
│   │       └── review-1.1.diff           il diff prima/dopo
...
| 2-3 | Il checkpoint umano | `chat/01`, la classifica dei reparti tolta |
| 3-4 | La story è un contratto, e cosa resta dopo | `1-1-…-di-una-sala.md`: AC, Task, Dev Agent Record |
| 4-5 | Lo stato non è nella tua testa | `sprint-status.yaml` |
| 5-8 | **Il pezzo forte**: la review a tre layer | `chat/09` + la sezione Review Findings della 1.1 |
...
Il `patch` della review 1.1: la query usava `stato <> 'disdetta'` invece di
`stato = 'attiva'`. Una prenotazione in `no_show` avrebbe continuato a occupare
